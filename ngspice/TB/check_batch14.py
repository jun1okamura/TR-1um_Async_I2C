#!/usr/bin/env python3
"""
check_batch14.py (2026-09-02)

Companion checker for tb_chip_i2c_batch14.spice (script/gen_chip_tb_
batch14_v9.py). Parses ngspice's own ".measure tran <name> ... " batch-mode
...
spice_batch14_expected.json (written alongside the .spice file by the same
generator run), and prints a PASS/FAIL summary in the same
"[t=...] OK/FAIL: <description>" format as irsim/run_tb.sh's own checker --
for a final, apples-to-apples cross-check against IRSIM (irsim_tb.cmd,
14/14 PASS) and Verilog (i2c_slave_async_tb.v / i2c_slave_async_net_tb.v,
14/14 PASS both).

Usage:
    cd ngspice/TB
    ngspice -b tb_chip_i2c_batch14.spice > spice_batch14.log 2>&1
...
def main():
    ...
    expected_path = Path(__file__).parent / "spice_batch14_expected.json"
