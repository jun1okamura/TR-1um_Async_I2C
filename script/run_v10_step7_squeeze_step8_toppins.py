"""
run_v10_step7_squeeze_step8_toppins.py (design_notes.md section 108.42/43)

STEP7 (channel squeeze) + STEP8 (top-pin routing against the squeezed
BBOX), applied to V10's final, fully-converged route
(layout/step10/v10_step_3_ripup_reroute.gds -- DRC 0, shorts 0,
108.40/108.41), in the same order run_v9_step7_squeeze_step6_toppins.py
uses (squeeze first, THEN route top pins against the new, smaller BBOX
-- v8/v9's own proven recipe). See that script's docstring for the full
rationale of deriving compaction_info empirically from the real M1
geometry rather than reusing a stale routing-time dump.

V10-specific difference from the v9 template: route_top_pins_nrow_fm.py
is called with net_file=FINAL so it resolves sda_oe/rx_valid's
assign-chain aliases (-> _187_/_154_, anonymous Yosys nets unique to
this netlist) via netlist_parser's own union-find resolver instead of
the static PORT_NET_ALIAS dict -- see highlight_top_pins_nrow_fm.py's
108.42 note. Without this, gather_pins() would silently find zero pins
for these two ports.
"""
import json
import subprocess
...
SCRIPT = "/sessions/dreamy-ecstatic-heisenberg/mnt/TR-1um_Async_I2C/script"
sys.path.insert(0, SCRIPT)

BASE = "/sessions/dreamy-ecstatic-heisenberg/mnt/TR-1um_Async_I2C"

PLACEMENT_JSON = BASE + "/LEF/placement_nrow_fm_v10.json"
NET_FILE = BASE + "/src/i2c_slave_async_net_v10_final.v"
IN_GDS = BASE + "/layout/step10/v10_step_3_ripup_reroute.gds"
PIN_MAP_IN = SCRIPT + "/pin_map_nrow_fm_v10_rr.json"
NET_SHAPES_IN = SCRIPT + "/net_shapes_nrow_fm_v10_rr.json"
...
COMPACTION_INFO = SCRIPT + "/compaction_info_nrow_fm_v10_empirical.json"
SQUEEZED_GDS = BASE + "/layout/step10/v10_step_7_squeezed.gds"
SQUEEZED_PIN_MAP = SCRIPT + "/pin_map_nrow_fm_v10sq.json"
SQUEEZED_NET_SHAPES = SCRIPT + "/net_shapes_nrow_fm_v10sq.json"

TOP_PINS_OUT_GDS = BASE + "/layout/step10/v10_step_8_squeezed_top_pins_routed.gds"
...
def step_top_pins(new_ch_heights):
    ...
    T.main(placement_json=PLACEMENT_JSON, in_gds=SQUEEZED_GDS, out_gds=TOP_PINS_OUT_GDS,
           ch_heights=new_ch_heights, net_shapes_json=SQUEEZED_NET_SHAPES, net_file=NET_FILE)
...
if __name__ == "__main__":
    ...
    print("=== sanity: base v10 recipe is still 0-short / DRC-clean ===")
    verify(IN_GDS, PIN_MAP_IN, orig_core_h, 1650.0, "v10 (pre-squeeze)")
    ...
    verify(SQUEEZED_GDS, SQUEEZED_PIN_MAP, new_core_h, 1650.0, "v10 squeezed")
    ...
    print("\n=== route top pins onto the new (squeezed) BBOX ===")
    step_top_pins(new_ch_heights)

    print("\n=== verify top-pin-routed squeezed result ===")
    verify(TOP_PINS_OUT_GDS, SQUEEZED_PIN_MAP, new_core_h2, 1650.0, "v10 squeezed + top-pins")
