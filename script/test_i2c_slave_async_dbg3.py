...
from i2c_slave_async_model import i2c_slave_async, STATE_NAMES
...
def testbench():
    ...
    def bus_model():
        ...
        while True:
            ...
            print(f'[t={now()}] bus_model: m_oe={int(sda_master_oe)} s_oe={int(sda_slave_oe)} -> sda_line.next={not (sda_master_oe or sda_slave_oe)}')
    ...
    def master():
        ...
        scl.next = 1
        yield delay(T)
        print(f"[t={now()}] PRE ack calc: sda_line={int(sda_line)} bool={bool(sda_line)} repr={sda_line!r}")
        ack = 0 if sda_line else 1
        print(f"[t={now()}] DEBUG ack(write addr)={ack} sda_line={int(sda_line)} sda_slave_oe={int(sda_slave_oe)}")
