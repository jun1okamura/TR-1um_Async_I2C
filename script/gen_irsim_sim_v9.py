...
SPICE = "../schematic/tr_1um_i2c_slave_async_v9_lvs.spice"
...
def find_pad_pullup_gate_node(devices, pad_net="P13"):
    ...
    # "P13" (the SDA pad, per gio_connections.json / v9_signal_routing_
    # plan.json) is the one we want. (MXM6, "NMOSE w=350u l=2u" with
    # gate tied straight to VSS, is a separate always-off ESD device on
    # the same PAD net -- excluded by the W match.)
...
def main():
    ...
    # output it inspects).
    pad_drive_node = find_pad_pullup_gate_node(devices, pad_net="P13")
    ...
    # level polarity.
    PULLUP_NODE = "P13"  # already a clean top-level net name in this source -- no aliasing needed
    ...
    with open(OUT_SIM, "w") as f:
        ...
        f.write("| schematic/tr_1um_i2c_slave_async_v9_lvs.spice (whole-chip,\n")
        f.write("| chip-level DRC/LVS-clean v9 reference -- design_notes.md 82-87).\n")
