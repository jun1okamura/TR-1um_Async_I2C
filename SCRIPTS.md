...
| `assign_v10_gio_pads.py` | V10の20信号ネットへのGIOパッド再割当（`scipy.optimize.linear_sum_assignment`による最適化、必要レーン数6——V9本体の10-11本より少ない）を`schematic/v10_signal_routing_plan.json`として確定。v2改訂でパッド共有グループの端子タイプ（O/I/B）誤りを修正（design_notes §108.55/108.57）。ユーザー要求により108.57案（レーン数最小だが見た目が入り組んだ対応）を撤回し、物理パッド番号昇順とbit番号が単調対応する**Option2**（`FORCED_BIT_PAD={0:3,1:4,2:5,3:6,4:11,5:12,6:13,7:14}`、必要レーン数12＝真の最小と同コスト）へ再修正（design_notes §108.69、現行の正式割当）。 |
| `route_gio_core_v10.py` | V10版GIO⇔コア間の結線ルータ（`route_gio_core_v9.py`をベースに`v10_signal_routing_plan.json`・V10再較正済み電源バスバー座標へ差し替え）。本セッションでHIZ1のDISチェーン誤登録・HIZ7/OUT2のVDD/VSSバス取り違えを修正し、ダイ内部を貫くカーテシアン配線ではなくDISチェーンと同じ`connect_gio_to_gio()`リング配線方式へ切替（108.63）。VDD/VSS電源バー⇔電源PAD間の10um配線を5本（2umスペース）へ拡張（108.64）。Option2パッド再割当（108.69）後も無修正のまま12レーンで再配線可能なことを確認済み。 |
| `finalize_chip_v10.py` | PTECT削除・RING_OSC配置・電源/信号配線（OUT/OUTD/ENB）・OpenSUSIロゴ埋め込みでチップを完成させる最終統合スクリプト（V9の`place_ring_osc.py`等をコア高さ非依存の幾何としてそのまま流用、ENBのみV10向けに再設計、design_notes §108.56/108.57）。本セッションでチップTOPセル直下のM2PIN(49,1)/TXM2(49,0)ピンマーカー16組の追加ステップを新規挿入（V9の`add_top_pins_gio_v9.py`相当が移植漏れだった、108.62——P9/P10 LVS NoMatchの真の根本原因）。108.69のOption2再割当でENBマージ先のハードコード座標（rst_nのレーン0前提・X=884.0固定の立ち上がり列）が実態と乖離し、P11/P12/P15のLVS不一致を引き起こしていたため、`v10_top_routed_net_shapes.json`からrst_n自身の実配線位置を毎回動的に読み取る方式（`RSTN_MERGE_X`動的算出）とENB立ち上がり列の空きスキャン（X=900.0へ変更）に修正——**実機KLayoutでチップレベルDRC/LVSクリーンを達成**（design_notes §108.70）。 |
...
| `gen_chip_tb_v10.py` | V10チップ全体のI2C WRITE→READ単発ngspiceテストベンチ（SCL=100kHz）。V10のパッド再割当（Option2、108.69）を反映した`TX_PADS`/`RX_NETS`等を実ネットリストから直接検証して確定（design_notes §108.66）。DIS(P7)を`vdis P7 0 DC {VDD}`固定から`PWL`によるダイナミック化（WRITE中LOW=ドライバ有効化、READ中HIGH=Hi-Z）へ変更し、`vtx0..7`もPADn直結ではなく`TXGATE`（DISと同位相のPWL）制御のSPICEスイッチ＋100kΩ直列抵抗経由に変更——WRITE中に本来有効化されるべきチップ自身のPADnドライバを実際にテストする、より設計意図に忠実な構成（design_notes §108.71/§108.72、tx_data_i=rx_data_iのWRITE中ループバックはユーザー確認済みの意図した設計）。 |
| `gen_chip_tb_batch14_v10.py` | プロジェクト既定の14項目バッチリグレッションテスト（WRITE→READ→不一致アドレス(0x11)NACK、IRSIM/Verilog RTL/Verilogネットリスト/V9 SPICEと同一14チェック）のV10版。`gen_chip_tb_v10.py`をimportし`RX_NETS`からbitマップを導出（design_notes §108.67）。companion checker: `ngspice/TB/check_batch14_v10.py`（`check_batch14.py`と同一ロジック、期待値JSONファイル名のみV10専用）。実機ngspiceでOption2パッド再割当・ENB修正後の**14/14 PASS**を確認したが、その後DATA_RD/WR値によって2/14 FAILする例を発見——実測の結果、原因はチップ側ではなく`.tran`のTmax（ソルバー間引き幅）が粗すぎたことによるアドレス比較ラッチの取り込みタイミング誤判定と判明、**50ns→1nsへ変更して解消**（design_notes §108.71、チップ設計そのものに欠陥は無いことを確定）。`gen_chip_tb_v10.py`と同じDIS/TXGATE動的化も反映済み（§108.72）。 |
