...
環境そのものが動くかどうかを切り分けたい場合は、IRSIM公式チュートリアルの
トランジスタ6個のAOIゲート例（本プロジェクトとは無関係）を`.sim`として
手元で作り`irsim <prm> <その.sim>`で試すとよい（本リポジトリには保持して
いない——プロジェクト固有の`sanity_check.sim`は使い捨ての動作確認用として
一度作られたのみで、現在は削除済み）。
